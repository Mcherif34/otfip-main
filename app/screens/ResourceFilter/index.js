...
import { PTaskPriority } from '@/data';
import { Button, Header, Icon, PaymentOption, SafeAreaView, Tag, Text } from '@/components';
import { getDocumentTypes, getResources } from '@/selectors';
...
const ResourceFilter = (props) => {
  ...
  useEffect(() => {
    ...
    setDocTypes(array);
    const arrayType = resourcesData.map((item) => {
      return {
        id: item.id,
        value: item.name,
        text: item.name,
      ...
      <View style={{ paddingHorizontal: 20, marginBottom: 20 }}>
        <Button
          ...
          onPress={() => {
            setLoading(true);
            setTimeout(() => {
              navigation.navigate('OfficialText', { format: format, type: type });
              setLoading(false);
            }, 300);
          }}
          loading={loading}
